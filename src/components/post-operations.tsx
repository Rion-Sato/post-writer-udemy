...
async function  deletePost(postId: string) {
    try {
        const responce = await fetch(`/api/posts/${postId}`, { method: "DELETE" });
        ...
        <div>
            <DropdownMenu>
                ...
                <DropdownMenuContent>
                    ...
                    <DropdownMenuItem onClick={() => setShowDeleteAlert(true)} className="text-destructive cursor-pointer focus:text-destructive">
                        削除
                    </DropdownMenuItem>
                </DropdownMenuContent>
            </DropdownMenu>
            <AlertDialog open={showDeleteAlert} onOpenChange={setShowDeleteAlert}>
                <AlertDialogContent>
                    ...
                    >
                        {isDeleteLoading ? <Icon.spinner className="animate-spin w-4 h-4 mr-2"/> : <Icon.trash className="w-4 h-4 mr-2" />}
                        削除
                    </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
